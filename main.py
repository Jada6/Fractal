...
class Spiral:
    def __init__(self, point, length):
        ...
        self.branch((point[0], point[1] - length), length,
                    -math.pi / 2 + self.ANGLE_CHANGE, True)

    def branch(self, point, length, angle, is_left):
        """ The recursive algorithm of the spiral:
        in each vertex, it continues with the spiral
        and creates a new smaller spiral with opposite direction """
        if length <= 1:
            return

        new_angle = (angle - self.ANGLE_CHANGE) if is_left \
            else (angle + self.ANGLE_CHANGE)
        new_point = move_point(point, length, new_angle)
        self.branches.append(Edge(length, point, new_point))

        self.branch(new_point,
                    length * self.LENGTH_CHANGE,
                    new_angle,
                    is_left)  # Continue with the spiral
        self.branch(new_point,
                    length * self.LENGTH_CHANGE / 3,
                    new_angle,
                    not is_left)  # New spiral
...
class Snowflake:
    ...
    def __init__(self, point, length):
        ...
        # changes the initial length of edges:
        self.LENGTH_FACTOR_1 = random.random()/2 + 1
        # changes the decreasing factor of edges:
        self.LENGTH_FACTOR_2 = random.random() + 0.8
    ...
    def star(self, point, length):
        ...
        for i in range(self.N_OF_INITIAL_WINGS):
            self.wing(point, length,
                      -math.pi / 2 + i * 2 * math.pi / self.N_OF_INITIAL_WINGS)
    ...
    def wing(self, point, length, angle):
        ...
        for i in range(self.N_OF_SUBWINGS):
            for side in (1, -1):
                new_point = move_point(point,
                                       length/2 + side * i * length / (2 * self.N_OF_SUBWINGS),
                                       angle)
                new_length = length * self.LENGTH_CHANGE\
                    / (self.LENGTH_FACTOR_1 + self.LENGTH_FACTOR_2 * i)
